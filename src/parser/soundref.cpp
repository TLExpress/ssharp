    ...
    while (std::getline(stream, path))
    {
        ...
        if (!std::regex_search(path, match, re))
        {
            throw ssexcept::parse_error(
                std::string{"Path contains invalid characters: "} + path);
        }
        path = match[1];
        ...
        set.insert({path_t(path), is_absolute_path_t, is_directory_t::file, hash});
    }
    return set;
}
