...
parsed_paths_t parse_buff(const buff_t& buff)
{
    ...
    for (uint32_t c = 0; c < path_count; c++)
    {
        ...
        if (d_offset >= size)
            throw ssexcept::invalid_input("not a valid pmd, out of bounds");
        ...
        set.emplace(parsed_path_t{path, true, false, std::nullopt});
    }
    return set;
}
